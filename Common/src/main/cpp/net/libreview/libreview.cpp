...
#endif
    ;
constexpr const int histelUitlen =
    sizeof(
        R"({"valueInMgPerDl":159.0,"extendedProperties":{"factoryTimestamp":"2022-06-20T20:24:33.000Z","isFirstAfterTimeChange":false},"recordNumber":-6629292502336270757,"timestamp":"2022-06-20T22:24:33.000+02:00"},)") +
    EXTRALEN;

constexpr const int startsensorlen =
    sizeof(
        R"({"type":"com.abbottdiabetescare.informatics.sensorstart","extendedProperties":{"factoryTimestamp":"2022-06-18T05:57:19.000Z"},"recordNumber":-6629292502336274432,"timestamp":"2022-06-18T07:57:19.000+02:00"})") +
    EXTRALEN;

// int64_t recordnumbermask=-6629292502336274432L;
// constexpr const  std::string_view
// datastart=R"({"DeviceData":{"deviceSettings":{"factoryConfig":{"UOM":")";
constexpr const std::string_view datastart =
    R"({"DeviceData":{"connectedDevices":{"insulinDevices":[]},"deviceSettings":{"factoryConfig":{"UOM":")";
// s/char\(.*\)\[\]=R\(.*\)$/constexpr  std::string_view \1=R\2/g
constexpr const std::string_view afterunit =
    R"("},"firmwareVersion":"2.12.0","miscellaneous":{"isStreaming":)";
constexpr const std::string_view afterstreaming = R"(,"selectedLanguage":")";
constexpr const std::string_view afterlocale =
    R"(","valueGlucoseTargetRangeLowInMgPerDl":)";
constexpr const std::string_view afterlow =
    R"(,"valueGlucoseTargetRangeHighInMgPerDl":)";
constexpr const std::string_view afterhigh = R"(,"selectedTimeFormat":")";
constexpr const std::string_view aftertimeformat =
    R"(hr","selectedCarbType":"grams of carbs"},"timestamp":")";
const constexpr std::string_view aftercurrenttime =
    R"("},"forceUpload":false,"header":{"device":{"hardwareDescriptor":")";
...
const constexpr std::string_view afterinsulin =
    R"(],"ketoneEntries":[],"scheduledContinuousGlucoseEntries":[)";
constexpr const std::string_view afterhists =
    R"(],"unscheduledContinuousGlucoseEntries":[)";
...
const constexpr std::string_view aftertoken =
    R"(","Domain":"Libreview","GatewayType":"FSLibreLink.Android"})";

inline static void valuestart(char *&ptr, int glvalue) {
  ptr += sprintf(
      ptr, R"({"valueInMgPerDl":%d,"extendedProperties":{"factoryTimestamp":")",
      glvalue);
}
...
extern LibreHist libreRealHistory(SensorGlucoseData *sens, uint32_t starttime,
                                  uint32_t nu);
...
static char *librehistel(const LibreHistEl *el, const int64_t histor,
                         char *buf) {
  char *ptr = buf;
  valuestart(ptr, el->mgdL);
  int mil = getmmsec();
  ptr += TdatestringGMT(el->ti, mil, ptr);
  static constexpr const std::string_view prerecord =
      R"(","isFirstAfterTimeChange":false},"recordNumber":)";
...
static char *libreScanel(const ScanData &scanel, const int16_t nr, char *ptr) {
  ...
  ptr += sprintf(
      ptr,
      R"(","trendArrow":"%s","isActionable":true,"isFirstAfterTimeChange":false},"recordNumber":%hd,"timestamp":")",
      trendName[scanel.tr], id);
...
static LibreHist librehistory(SensorGlucoseData *sensdata, uint32_t starttime,
                              uint32_t nu) {
  const int notsend = sensdata->getinfo()->libreviewnotsend;
#ifndef NOLOG
  const time_t tim = starttime;
  LOGGER("%s librehistory notsend=%d from %s",
         sensdata->shortsensorname()->data(), notsend, ctime(&tim));
#endif
  ...
  if (left != periodright) {
    int toid = laststreamid - ((left > periodright)
                                   ? (left - periodright)
                                   : (periodmin - (periodright - left)));
    while (true) {
      --laststream;
      if (laststream < firststream) {
  ...
  const ScanData *found =
      (firststream->t < starttime)
          ? sensdata->firstnotless({firststream, leftstream}, starttime)
          : firststream;
  int startid = 0;
  if (found > &stream[0]) {
  ...
  return {.histor = histor,
          .list = list,
          .size = datuit,
          .starttime = sensdata->getstarttime(),
          .msec = 0,
          .notsend = newnotsend,
          .notsendHistory = notsendHistory,
          .sendstart = sensdata->getinfo()->sendsensorstart};
}
...
//{"type":"com.abbottdiabetescare.informatics.sensorstart","extendedProperties":{"factoryTimestamp":"2023-09-09T20:16:00.000Z","gmin":"40","gmax":"500","latejoin":"false","puckGen":"3","wearDuration":"20160","warmupTime":"60","isStreaming":"true","productType":"3"},"recordNumber":-6629287740365012992,"timestamp":"2023-09-09T22:16:00.000+02:00"}
static int mksensorstart(int64_t recordnumbermask, time_t starttime, int msec,
                         char *buf) {
  static constexpr const char start[] =
      R"({"type":"com.abbottdiabetescare.informatics.sensorstart","extendedProperties":{"factoryTimestamp":")";
  ...
  static const char afterstartimeGMT[] =
      R"(","gmin":"40","gmax":"500","latejoin":"false","puckGen":"3","wearDuration":"20160","warmupTime":"60","isStreaming":"true","productType":"3"},"recordNumber":)";
...
static int writelist(const LibreHist *list, const int len, char *buf,
                     int (*func)(const LibreHist &el, char *buf)) {
  char *bufptr = buf;
...
constexpr const int scanelsize = sizeof(
    R"({"valueInMgPerDl":500.0,"extendedProperties":{"factoryTimestamp":"2022-06-20T16:26:06.000Z","trendArrow":"FallingQuickly","isActionable":true,"isFirstAfterTimeChange":false},"recordNumber":10256,"timestamp":"2022-06-20T18:26:06.000+02:00"},)");
#include <jni.h>
...
bool libresendmeasurements(bool libre3, const char *measurements,
                           const int len) {
  static jmethodID postmeasurements =
      getenv()->GetStaticMethodID(libreviewclass, "postmeasurements", "(Z[B)Z");
  auto env = getenv();
  jbyteArray uit = env->NewByteArray(len);
  env->SetByteArrayRegion(uit, 0, len, (const jbyte *)measurements);
  bool res = env->CallStaticBooleanMethod(libreviewclass, postmeasurements,
                                          libre3, uit);
...
static char *onecurrent(const ScanData &scanel, const int nr, char *ptr,
  ...
  const char currentformat[] =
      R"(","isViewed":"%s","lowOutOfRange":"%s","highOutOfRange":"%s","trendArrow":"%s","isActionable":"true","isFirstAfterTimeChange":"false"},"recordNumber":%d,"timestamp":")";
  const int id = nr * 256;
  ptr += sprintf(ptr, currentformat, isviewed ? "true" : "false",
                 mgdL < 40 ? "true" : "false", mgdL > 500 ? "true" : "false",
                 trendName[scanel.tr], id);
  ptr += Tdatestringlocal(scanel.gettime(), mil, ptr);
  addar(ptr, R"("})");
  return ptr;
...
static void addoldcurrents(char *&uitptr, const SensorGlucoseData *sens,
...
static int addcurrents(char *&uitptr, time_t nu,
...
bool sendlibreviewdata(const int newcurrent, const uint32_t nu) {
  ...
  uint32_t starttime = (settings->data()->lastlibretime)
                           ? (settings->data()->lastlibretime)
                           : (nu - librekeepsecs);
  ...
  LOGGER("start %d sensors sendlibreviewdata librekeepsecs=%d from %s", senslen,
         librekeepsecs, ctime(&tim));
#endif
      ...
      if (!info->libreviewsendall) {
        if (sensdata->isLibre2()) {
          ...
          const bool userealhistory = (info->startedwithStreamhistory &&
                                       (info->libreviewnotsendHistory >=
                                            info->startedwithStreamhistory ||
                                        info->startedwithStreamhistory == 1)) ||
                                      !sensdata->pollcount();
          switchrealhistory(sensdata, userealhistory);
          const auto el = lists[i] =
              (userealhistory ? libreRealHistory
                              : librehistory)(sensdata, starttimeiter, nu);
          ...
          if (!info->newscan || lastscan > sensdata->scancount()) {
            ...
            const ScanData *found =
                sensdata->firstnotless({startscans, endscans}, after);
            int index = found - startscans;
            scanoff = info->scanoff = index - info->libreviewScan;
            info->newscan = true;
            lastscan = scanoff + startid;
          }
          LOGGER("startedwithStreamhistory=%d scanoff=%d startid=%d "
                 "lastscan=%d scancount=%d\n",
                 info->startedwithStreamhistory, scanoff, startid, lastscan,
                 sensdata->scancount());
  ...
  constexpr const int timeuitlen =
      sizeof(R"(2022-06-21T00:26:21.098+02:00)") - 1;
  constexpr const int deviceidlen =
      sizeof(R"(b76f19a9-d4a0-4d67-8999-56b89b0968ee)") - 1;
  ...
  int totallen =
      bytesnumbers + histtotal * histelUitlen + startsensorlen * senslen +
      datastart.size() + unitlen + afterunit.size() + afterstreaming.size() +
      afterlocale.size() + afterlow.size() + 10 + afterhigh.size() + 4 +
      aftertimeformat.size() + 6 + timeuitlen +
      // s/addstrview(uitptr,\(.*\));/\1.size()+/g
      (aftercurrenttime.size() + dMODEL.size() + afterHardDescr.size() +
       dMANUFACTURER.size() + afterHardwareName.size() + aftermodelName.size() +
       5 + afterosVersionName.size() + appstart.size() + sizeof(countries[0]) +
       deviceidlen + afterident.size()) *
          2 +
  ...
  LOGGER("usertokenlen=%d histtotal=%d histelUitlen=%d senslen=%d nrscans=%u "
         "scanelsize=%d totallen=%d nu=%u \n",
         usertokenlen, histtotal, histelUitlen, senslen, nrscans, scanelsize,
         totallen, nu);
#endif
  ...
  if (histtotal) {
    uitptr =
        writelibrehists(lists + startsensor, senslen - startsensor, uitptr);
  }
    ...
    for (int i = last; i >= startsensor; i--) {
      ...
      if (!sensdata->getinfo()->libreviewsendall) {
        ...
        LOGGER("scanoff=%d startid=%d end=%d\n", scanoff, startid,
               scans.size());
        for (const ScanData *iter = &scans[scanoff + startid]; iter < endscans;
             ++iter) {
          ++id;
...
static bool initlibreconfig(bool libre3, bool restart) {
  static jmethodID libreconfig =
      getenv()->GetStaticMethodID(libreviewclass, "libreconfig", "(ZZ)Z");
  return getenv()->CallStaticBooleanMethod(libreviewclass, libreconfig, libre3,
                                           restart);
}
/*
static bool putsensor(const char *sensorid) {
    JNIEnv *env=getenv();
    static jmethodID
putsensor=env->GetStaticMethodID(libreviewclass,"putsensor","(Ljava/lang/String;)Z");
...
static bool putsensor(bool libre3, const char *sensorid) {
  JNIEnv *env = getenv();
  static jmethodID putsensor =
      env->GetStaticMethodID(libreviewclass, "putsensor", "(Z[B)Z");
  jbyteArray message = libre3 ? getlibre3puttext(env, sensorid)
                              : getlibre2puttext(env, sensorid);
  bool res =
      env->CallStaticBooleanMethod(libreviewclass, putsensor, libre3, message);
...
int askhasnewcurrent(time_t nu) {
    ...
    for(int i=last;i>=0;--i) {
      ...
      if(sens->isSibionics())
         continue;
         int
start=sens->isLibre3()?sens->getinfo()->libreviewScan:sens->getinfo()->libreCurrentIter;
        int pollstart= sens->getinfo()->pollstart;
        if(start<pollstart)
            start=pollstart;
        const int        ends=sens->pollcount()-1;
...
void libreviewthread() {
  ...
  while (true) {
    if (!networkpresent || !librecondition.dobackup) {
      ...
      auto status =
#endif
          librecondition.backupcond.wait_until(
              lck, now + std::chrono::minutes(waitmin));
    ...
    if ((librecondition.dobackup & Backup::wakeall) ||
        (hasnewcurrent && (librecondition.dobackup & Backup::wakestream))) {
      librecondition.dobackup = 0;
      ...
      if (settings->data()->haslibre2) {
        ...
        if (!settings->data()->libreinit) {
          if (!(settings->data()->libreinit =
                    initlibreconfig(false, alwaysnewstatus))) {
...
#ifndef NOLOG
      LOGGER("clearfromDate Libre3 index=%d notsend=%d stream=%d from=%s",
             index, histpos, streampos, ctime(&tim));
#endif
    } else {
...
#ifndef NOLOG
        const time_t tim = starttime;
        LOGGER("clearfromDate Libre2 index=%d notsend=%d stream=%d scanpos=%d "
               "from=%s",
               index, histpos, streampos, scanpos, ctime(&tim));
#endif
      }
    }
...
extern "C" JNIEXPORT void JNICALL fromjava(clearlibreFromMSec)(JNIEnv *env,
                                                               jclass clazz,
                                                               jlong fromtime) {
  if (fromtime)
...
extern "C" JNIEXPORT void JNICALL fromjava(wakelibreview)(JNIEnv *env,
                                                          jclass clazz,
                                                          int minutes) {
  if (networkpresent)
...
extern "C" JNIEXPORT jboolean JNICALL fromjava(getuselibreview)(JNIEnv *env,
                                                                jclass cl) {
  //     return settings->data()->uselibre;
  return settings->data()->sendtolibreview;
}

extern "C" JNIEXPORT void JNICALL fromjava(setuselibreview)(JNIEnv *env,
                                                            jclass cl,
                                                            jboolean val) {
...
extern "C" JNIEXPORT void JNICALL fromjava(askServerforAccountID)(JNIEnv *env,
                                                                  jclass cl) {
  alwaysnewstatus = true;
  askforaccount = true;
  settings->data()->haslibre3 = true;
...
// extern "C" JNIEXPORT void JNICALL fromjava(setDevice) (JNIEnv *env, jclass
// clazz, jstring jMANUFACTURER, jstring jMODEL, jstring jRELEASE) {
