...
struct glucoseinput {
  ...
  int getpredictedmgdL() const { return predictedmgdL; }
  ...
  void showglucose(const time_t starttime) const {
    ...
    // LOGGER("type=%02x status=%d started %6d ago %.24s glucose=%3d %4.1f
    // trend=% 3.2f predicted=%3d %4.1f state=%d info=%d
    ...
    LOGGER("type=%02x status=%d sincestart=%d %.24s %2d ago glucose=%3d %4.1f "
           "trend=% 3.2f predicted=%3d %4.1f state=%d info=%d unknown=%d\n",
           glucose->type, glucose->status, secsSinceStart, ctime(&then), age,
           glu, glu / 18.0f, ((float)glucose->trend) / 10.0f, pre, pre / 18.0f,
           state, info, unknown);
  };
#endif

  float getRateofChange() const { return (float)trend / 10.0f; }

  void save(SensorGlucoseData *sens, uint32_t wastime, int index) const {
    const auto rate = getRateofChange();
    sens->savepollallIDs<DEXSECONDS>(wastime, index, mgdL,
                                     rate2changeindex(rate), rate);
  }

  void actual(SensorGlucoseData *sens, jlong *timeres,
    ...
    if (wasstart < 1605654000 || wasstart >= nowsec || !sens->pollcount()) {
      sensors->getsensor(sensorindex)->starttime = sens->getinfo()->starttime =
          nowsec - secsSinceStart;
      sensors->setindices();
...
fromjava(dexcomProcessData)(JNIEnv *envin, jclass _, jlong dataptr,
                            jbyteArray bluetoothdata, jlongArray jtimeres) {
  ...
  const glucoseinput *glucose =
      reinterpret_cast<decltype(glucose)>(bluedata.data());
...
/*
extern "C" JNIEXPORT  jbyteArray  JNICALL   fromjava(getDexbackfillcmd)(JNIEnv
*envin, jclass _cl,jlong dataptr) { dexcomstream
*sdata=reinterpret_cast<dexcomstream *>(dataptr); SensorGlucoseData
*sens=sdata->hist; auto *info=sens->getinfo(); const int last=info->pollcount-1;
   ...
   if(last>=0) {
        ...
        if(end<=start) {
          ...
          ; //After a backfill it seems to have less missing connections. So it
always ask for backfill for the far future.
          }
        #ifndef NOLOG
        char buf1[27],buf2[27];
        LOGGER("getDexbackfillcmd %d-%d
%.23s-%23s\n",end,start,ctime_r(&starts,buf1), ctime_r(&ends,buf2)); #endif
        }
      else  {
        ...
        jbyteArray uit=envin->NewByteArray(len);
         ...
         envin->SetByteArrayRegion(uit,0,len,reinterpret_cast<const
jbyte*>(&data));
...
static jbyteArray mkbackfillcmd(JNIEnv *envin, int start, int end) {
  ...
  jbyteArray uit = envin->NewByteArray(len);
  struct askfilldata data{.starttime = start, .endtime = end};
  envin->SetByteArrayRegion(uit, 0, len,
                            reinterpret_cast<const jbyte *>(&data));
  return uit;
}
extern "C" JNIEXPORT jbyteArray JNICALL
fromjava(getDexbackfillcmd)(JNIEnv *envin, jclass _cl, jlong dataptr) {
  dexcomstream *sdata = reinterpret_cast<dexcomstream *>(dataptr);
  SensorGlucoseData *sens = sdata->hist;
...
#ifndef NOLOG
    char buf1[27], buf2[27];
    LOGGER("getDexbackfillcmd %d-%d %.24s-%s", end, start,
           ctime_r(&starts, buf1), ctime_r(&ends, buf2));
#endif
...
struct dexbackfill {
  ...
  [[nodiscard]] int getpredictedmgdL() const { return 0; }
  [[nodiscard]] int getindex() const { return secsSinceStart / DEXSECONDS; }
  ...
  [[nodiscard]] float gettrend() const { return ((float)trend) / 10.0f; }
  /*
void showdata(const uint32_t sensorstart) const {
  time_t was=gettime(sensorstart);
  LOGGER("%06d %03d %4.1f % 2.1f
%s",secsSinceStart,mgdL,(float)mgdL/18.0,gettrend(),ctime(&was));
  }; */
  ...
  void backfill(SensorGlucoseData *sens) const {
    ...
    if (id < sens->maxstreampos()) {
      ...
      if (id < sens->getbroadcastfrom())
        sens->setbroadcastfrom(id);
      sens->backstream(id);
...
extern bool validate3(const EC_POINT *g1, const EC_POINT *g2,
                      const PCert &cert1, const PCert &cert3);

// Saves received keys from other side;

std::array<uint8_t, 32> mkSharedKey(const PCert &cert2, const PCert &cert3,
                                    const BIGNUM *pass, const BIGNUM *x2);
extern "C" JNIEXPORT jboolean JNICALL fromjava(dexPutPubKey)(
    JNIEnv *envin, jclass cl, jlong dataptr, int which, jbyteArray input) {
  if (which < 0 || which > 2) {
  ...
  if (validate3(sdata->dexcontext.key[0].getpublic(),
                sdata->dexcontext.key[1].getpublic(), certs[0], certs[2])) {
    auto key = mkSharedKey(certs[1], certs[2], sdata->dexcontext.pin,
                           sdata->dexcontext.key[1].getprivate());
    SensorGlucoseData *sens = sdata->hist;
...
template <typename T> jbyteArray getjbyteArray(JNIEnv *env, const T &cont) {
  static_assert(sizeof(cont[0]) == sizeof(jbyte));
  jbyteArray uit = env->NewByteArray(cont.size());
  env->SetByteArrayRegion(uit, 0, cont.size(),
                          reinterpret_cast<const jbyte *>(cont.data()));
  return uit;
}

extern bool makeRound3Cert(PCert &cert, const EC_POINT *pub1,
                           const EC_POINT *pub2, const EC_POINT *pubA,
                           const BIGNUM *privB, const BIGNUM *pass);
extern "C" JNIEXPORT jbyteArray JNICALL
fromjava(makeRound3bytes)(JNIEnv *envin, jclass cl, jlong dataptr) {
  if (!dataptr) {
  ...
  PCert *certs = sdata->dexcontext.certs;
  PCert certuit3;
  if (!makeRound3Cert(certuit3, certs[0].pubkey1, certs[1].pubkey1,
                      sdata->dexcontext.key[0].getpublic(),
                      sdata->dexcontext.key[1].getprivate(),
                      sdata->dexcontext.pin)) {
...
extern "C" JNIEXPORT jbyteArray JNICALL
fromjava(makeRound12bytes)(JNIEnv *envin, jclass cl, jlong dataptr, int which) {
  if (!dataptr) {
  ...
  const KeyPair &key =
      reinterpret_cast<dexcomstream *>(dataptr)->dexcontext.key[which];
  if (key.invalid()) {
    LOGAR("makeRound12bytes key.invalid()");
    return nullptr;
  }
  return getjbyteArray(
      envin, mkround12(key)); // TODO make mkround12 not copy unnecessarily
}

extern std::array<uint8_t, 64> getchallenge(std::span<const uint8_t> input);
extern "C" JNIEXPORT jbyteArray JNICALL
fromjava(dexChallenger)(JNIEnv *envin, jclass cl, jbyteArray jdata) {
  LOGAR("dexChallenger");
  const auto arlen = envin->GetArrayLength(jdata);
  //  const CritAr  bytes(envin,jdata);
  const uint8_t *bytes =
      (const uint8_t *)envin->GetPrimitiveArrayCritical(jdata, nullptr);
...
extern "C" JNIEXPORT jboolean JNICALL fromjava(isAuthenticated)(JNIEnv *envin,
                                                                jclass cl,
                                                                jlong dataptr) {
  const dexcomstream *sdata = reinterpret_cast<dexcomstream *>(dataptr);
  const SensorGlucoseData *sens = sdata->hist;
  jboolean res = *reinterpret_cast<const unsigned long long *>(
                     sens->getinfo()->sharedKey.data()) ||
                 sdata->dexcontext.certs[2].valid();
  LOGGER("isAuthenticated()=%d\n", res);
  return res;
}

extern bool encrypt8AES(const uint8_t *keybytes, const uint8_t *data,
                        unsigned char *uit);
extern "C" JNIEXPORT jboolean JNICALL
fromjava(dex8AES)(JNIEnv *envin, jclass cl, jlong dataptr, jbyteArray jdata,
                  jint startdat, jbyteArray jout, jint startout) {
  const SensorGlucoseData *sens =
      reinterpret_cast<dexcomstream *>(dataptr)->hist;
  const CritAr bytes(envin, jdata);
  CritArSave out(envin, jout);
  return encrypt8AES(sens->getinfo()->sharedKey.data(),
                     reinterpret_cast<const unsigned char *>(bytes.data()) +
                         startdat,
                     reinterpret_cast<unsigned char *>(out.data()) + startout);
}
extern "C" JNIEXPORT void JNICALL fromjava(dexResetKeys)(JNIEnv *envin,
                                                         jclass cl,
                                                         jlong dataptr) {
  dexcomstream *sdata = reinterpret_cast<dexcomstream *>(dataptr);
  SensorGlucoseData *sens = sdata->hist;
  ...
  *reinterpret_cast<unsigned long long *>(sens->getinfo()->sharedKey.data()) =
      0LL;
}

// Large pin possible?
extern "C" JNIEXPORT jint JNICALL fromjava(getDexCertSize)(JNIEnv *envin,
                                                           jclass cl,
                                                           jbyteArray jar) {
...
extern "C" JNIEXPORT void JNICALL fromjava(dexSaveDeviceName)(
...
extern "C" JNIEXPORT jstring JNICALL fromjava(dexGetDeviceName)(JNIEnv *env,
                                                                jclass cl,
                                                                jlong dataptr) {
  if (!dataptr)
    return nullptr;
  const char *name = reinterpret_cast<const streamdata *>(dataptr)
                         ->hist->getinfo()
                         ->DexDeviceName;
  if (*name)
...
extern "C" JNIEXPORT jboolean JNICALL fromjava(dexKnownSensor)(JNIEnv *env,
                                                               jclass cl,
                                                               jlong dataptr) {
  if (!dataptr)
    return false;
...
extern "C" JNIEXPORT jboolean JNICALL
fromjava(dexCandidate)(JNIEnv *env, jclass cl, jlong dataptr,
                       jstring jdeviceName, jstring jaddress) {
  if (!jaddress) {
  ...
  destruct _1([jdeviceName, deviceName, env]() {
  ...
  destruct _2([jaddress, address, env]() {
  ...
  if (!sensors->knownDex(deviceName, address)) {
    ...
    if (timeback > 30 * 60) {
      ...
      const address_t &candAddress =
          *reinterpret_cast<const address_t *>(address);
        ...
        if (endAddress >= startAddress) {
          if (now > sens->usedAddressesTime) {
            if (*endAddress == candAddress) {
              LOGGER(
                  "%.4s dexCandidate Skip previous sensor %s, tried too long\n",
                  pin, address);
              return false;
            } else
            ...
            LOGGER(
                "%.4s dexCandidate tried %s not so long now (%u) waitfor %u %s",
                pin, endAddress->data(), now, tim, ctime(&tim));
          }
          if (endAddress > startAddress) {
            auto hit = std::find(startAddress, endAddress,
                                 *reinterpret_cast<const address_t *>(address));
            if (hit < endAddress) {
