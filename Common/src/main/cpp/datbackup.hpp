...
extern void startReceiverThread(int allindex);
...
int updatenums(crypt_t *, Connect *connect, struct changednums *nums, int ind);
...
struct updateone {
  ...
  template <typename Self> auto *getConnect(this Self &&self) {
...
struct updatedata {
  ...
  void wakesender(uintptr_t kind = 8 /*wakeall*/);
...
public:
  static constexpr const uintptr_t wakestream = ::wakestream;
  static constexpr const uintptr_t wakescan = ::wakescan;
  static constexpr const uintptr_t wakenums = ::wakenums;
  static constexpr const uintptr_t wakeall = ::wakeall;
  static constexpr const uintptr_t wakestop = ::wakestop;
  static constexpr const uintptr_t wakeother = ::wakeother;
  static constexpr const uintptr_t wakeend = ::wakeend;
  ...
  struct updatedata *getupdatedata() {
    return reinterpret_cast<struct updatedata *>(mapdata.data());
  }
  const struct updatedata *getupdatedata() const {
    return reinterpret_cast<const struct updatedata *>(mapdata.data());
  }
  ...
  const char *getmyport() const { return getupdatedata()->port; }
  void startreceiver(bool always = false) {
    if (receiveractive() && !always)
      return;
    ::stopreceiver();
    ::startreceiver(getupdatedata()->port, getupdatedata()->allhosts,
                    getupdatedata()->hostnr);
  }
  void stopreceiver() {
    ::stopreceiver();
  ...
  ~Backup() { wakebackuponly(wakestop | wakeend); }
#include <netdb.h>
#include <sys/socket.h>
  ...
  void addsize() {
    if (getupdatedata()->sendnr >= maxsendtohost) {
      ...
      int newlen = sizeof(updatedata) +
                   (newsendnr - maxsendtohost) * sizeof(struct updateone);
      int len = mapdata.size();
      if (newlen > len) {
        mapdata.extend(
            globalbasedir, backupdat,
            newlen); // Goes wrong because of existing pointers to old data
      }
    }
  }
  void deupdated() {
  ...
  void deletestart(int sendindex) {
    ...
    if (--getupdatedata()->sendnr != sendindex) {
      ...
      memmove(getupdatedata()->tosend + sendindex,
              getupdatedata()->tosend + sendindex + 1,
              fromend * sizeof(getupdatedata()->tosend[0]));
      delete crypts[sendindex];
      memmove(&crypts[sendindex], &crypts[sendindex + 1],
              fromend * sizeof(crypts[0]));
      crypts[getupdatedata()->sendnr] = nullptr;
      LOGGER("crypts[%d]=nullptr\n", getupdatedata()->sendnr);
    }
    if (con_vars[getupdatedata()->sendnr])
      con_vars[getupdatedata()->sendnr]->wakebackuponly(Backup::wakestop |
                                                        Backup::wakeend);
  }
  ...
  void deletehost(int index) {
    ...
    if (--getupdatedata()->hostnr != index) {
      ...
      memmove(getupdatedata()->allhosts + index,
              getupdatedata()->allhosts + index + 1,
              fromend * sizeof(getupdatedata()->allhosts[0]));
      memmove(hostsocks + index, hostsocks + index + 1,
              fromend * sizeof(hostsocks[0]));
      memmove(lastuptodate + index, lastuptodate + index + 1,
              fromend * sizeof(lastuptodate[0]));
    }
  ...
  void changereceiver(int allindex, int index, const bool sendnums,
                      const bool sendstream, const bool sendscans,
                      const bool restore, const bool haspass,
                      const uint32_t starttime) {
    LOGGER("changereceiver(allindex=%d,index=%d,sendnums=%d,sendstream=%d,"
           "sendscans=%d,haspass=%d,starttime=%u) sendnr=%d\n",
  ...
  static constexpr const std::array<uint8_t, 16> remix = {
      0x19, 0xED, 0xA0, 0x4A, 0x94, 0x9D, 0x0C, 0xD7,
      0x82, 0x4A, 0x74, 0xA9, 0x0E, 0x71, 0x84, 0x8B};
  //,0x87,0x7F,0x0F,0xD7,0xA8,0xEE,0x8C,0xD7,0x80
  static void setpass(std::array<uint8_t, 16> &passuit,
                      const string_view passin) {
    int len = passin.size();
    if (!len) {
      passuit = {};
      return;
  ...
  static const std::array<char, 17>
  passback(const std::array<uint8_t, 16> &passin) {
  ...
  /*
  void resethost(int index) {
      if(index<getupdatedata()->hostnr&&getupdatedata()->allhosts[index].index>=0)
  {  //Fout?? int tohost=getupdatedata()->allhosts[index].index;
          clearhost(tohost);
          }
      } */

  int jsetips(const char *port, JNIEnv *env, const jobjectArray jar,
              const int len, struct sockaddr_in6 *connect, const int lmaxip) {
  ...
  int setips(const char *port, const char **names, const int len,
             struct sockaddr_in6 *connect, const int lmaxip) {
    int uselen = std::min(lmaxip, len);
    for (int i = 0; i < uselen; i++) {
      LOGGER("setip %s\n", names[i]);
  ...
  int changehost(int index, JNIEnv *env, jobjectArray jnames, int nr,
                 bool detect, string_view port, const bool sendnums,
                 const bool sendstream, const bool sendscans,
                 const bool restore, const bool receive, const bool activeonly,
                 string_view pass, uint32_t starttime, bool passiveonly,
                 const char *label = nullptr, const bool testip = true,
                 bool startthreads = true, bool hashostname = false) {
    const int hostnr = getupdatedata()->hostnr;
    LOGGER(
        "hostnr=%d "
        "changehost(%d,sendnums=%d,sendstream=%d,sendscans=%d,receive=%d,"
        "activeonly=%d,passiveonly=%d,label=%s port=%s nr=%d hashostname=%d\n",
        hostnr, index, sendnums, sendstream, sendscans, receive, activeonly,
        passiveonly, label, port.data(), nr, hashostname);
    if (index < 0)
      index = hostnr;
...
>>>>>>> rebase/test-1.0.4-merge
    ...
    if (sendto) {
      ...
      changereceiver(index, tohost, sendnums, sendstream, sendscans, restore,
                     pass.size(), starttime);
    ...
    if (hashostname) {
      ...
      if (env) {
        ...
        if (namelen >=
            passhost_t::hostnamedata::maxhostname) { // terminanting zero
          LOGGER("supplied name too long %d>%d\n", namelen,
                 passhost_t::hostnamedata::maxhostname);
          if (newhost && sendto) {
            --getupdatedata()->sendnr;
          }
          return -5;
      ...
      res = thehost.nr =
          env ? jsetips(port.data(), env, jnames, nr, thehost.ips, lmaxip)
              : setips(port.data(), (const char **)jnames, nr, thehost.ips,
                       lmaxip);
    }
    int ret = index;
    if (res < 0) {
    ...
    if (detect) {
      if (res < lmaxip) {
        thehost.detect = true;
        thehost.ips[res] = {.sin6_family = AF_INET6,
                            .sin6_port = htons(atoi(port.data())),
                            .sin6_addr = noaddress};
      } else
    ...
    */
    thehost.receivefrom =
        receive ? (reconnect ? 3 : 2) : ((sendto & reconnect) ? 1 : 0);
  ...
  void notupdatedsettings() { wakebackup(wakeall); }
  struct changednums *getnums(int hostindex, int numindex) {
    return getupdatedata()->tosend[hostindex].nums + numindex;
  }
  int gethostnr() const { return getupdatedata()->hostnr; }
  int getsendhostnr() const { return getupdatedata()->sendnr; }
  // int histnr=0;
  ...
  bool sendwakesender(int h) {
    ...
    if (auto *con = shost.getConnect()) {
      if (con->finish) {
        LOGGER("sendwakesender: %d finish\n", h);
        return false;
      }
  ...
  /*
extern int updateproc(condvar_t *varsptr,uintptr_t cond,updateone &shost,int
(updateone::*proc)( )) ; extern void    endbackupthread(int h) ; extern void
doupdates(const uintptr_t current,const int h) ; extern void lockwait(uintptr_t
&current,int h) ; extern void        notpassive(uintptr_t current,int
sendindex); extern bool doend(int sendindex); */
  void backupthread(int allindex, int sendindex) {
    ...
    LOGGER("%d backupthread, wearos=%d con_vars=%p sock=%i %p\n", allindex,
           getupdatedata()->allhosts[allindex].wearos, con_vars[sendindex],
           host.getsock(), host.getcrypt());
  ...
  int updateproc(condvar_t *varsptr, uintptr_t cond, updateone &shost,
                 int (updateone::*proc)()) {
  ...
  void doupdates(const uintptr_t current, const int h) {
    ...
    (didnums =
         updateproc(con_vars[h], current & wakenums, getupdatedata()->tosend[h],
                    &updateone::updatenums)) &&
        (didstream = updateproc(con_vars[h], current & wakestream,
                                getupdatedata()->tosend[h],
                                &updateone::updatestreamu)) &&
        (didscans = updateproc(con_vars[h], current & wakescan,
                               getupdatedata()->tosend[h],
                               &updateone::updatescansu)) &&
        (didupdate =
             updateproc(con_vars[h], current & wakeall,
                        getupdatedata()->tosend[h], &updateone::update));
    const uint16_t command =
        ((didnums & 1) ? wakenums : 0) | (didstream & 1 ? wakestream : 0) |
        (didscans & 1 ? wakescan : 0) | (didupdate & 1 ? wakeall : 0);
    if (command) {
  ...
  void lockwait(uintptr_t &current, int h) {
    ...
    con_vars[h]->backupcond.wait(lck,
                                 [h] { return backup->con_vars[h]->dobackup; });
  ...
  typedef std::conditional<sizeof(unsigned long) == sizeof(uintptr_t),
                           unsigned long, uintptr_t>::type myuintptr_t;
  // TO get rid of stupid clang++ warnings, that I shouldnt specify %lx  even if
  ...
  inline std::span<passhost_t> getHosts() {
    return {
        getupdatedata()->allhosts,
        static_cast<std::span<passhost_t>::size_type>(getupdatedata()->hostnr)};
  }
};
...
inline std::span<passhost_t> getBackupHosts() { return backup->getHosts(); }
